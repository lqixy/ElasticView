...
namespace ElasticView.AppService
{
    public class ElasticAppService : IElasticAppService
    {
        ...
        public async Task<Contracts.Summary> GetHealth(string url)
        {
            if (string.IsNullOrWhiteSpace(url)) return new Contracts.Summary();

            var client = GetClient(url);

            var indicesInfo = await GetIndices(client);

            //var fs = client.Nodes.Stats(x => x.FilterPath("nodes.*.fs"));

            var clusterInfo = await GetClusterInfo(client);
            //var indices = client.Cat.Indices();
            var indexInfos = await GetIndexInfos(client);

            return new Contracts.Summary(indicesInfo, clusterInfo);
        }
        ...
        private async Task<ClusterHealthInfo> GetClusterHealth(ElasticClient client)
        {
            ...
            var healthInfo = new ClusterHealthInfo(health.Status.ToString(), health.TimedOut, health.NumberOfNodes, health.NumberOfDataNodes,
